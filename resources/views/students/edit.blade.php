    ...
    <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
          ...
          <div class="mt-5 md:mt-0 md:col-span-2">
            <form action="/students/{{ $studentFind->id }}" method="POST">
              @method('put')
                @csrf
                  ...
                  <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 sm:col-span-3">
                      <label for="name" class="block text-sm font-medium text-gray-700">Name Student</label>
                        ...
                        @enderror
                      <input required type="text" name="name" id="name" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full h-10 px-2 shadow-md sm:text-sm border-gray-300 rounded-md" value="{{ old('name', $studentFind->name) }}">
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                      <label for="nis" class="block text-sm font-medium text-gray-700">NIS</label>
                        @error('nis')
                            <span class="mt-2 text-center text-sm text-red-600">{{ $message }}</span>
                        @enderror
                      <input required type="number" name="nis" id="nis" autocomplete="address-level2" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full h-10 px-2 shadow-md sm:text-sm border-gray-300 rounded-md" value="{{ old('nis', $studentFind->nis) }}">
                    </div> 
