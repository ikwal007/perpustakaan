    ...
    <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
          <div class="md:col-span-1">
            <div class="px-4 sm:px-0">
              <div class="ml-4 divide-y divide-gray-200">
                <h3 class="text-lg font-medium leading-6 text-gray-900">Edit categories information</h3>
          ...
          <div class="mt-5 md:mt-0 md:col-span-2">
            <form action="/categories/{{ $categoryFind->id }}" method="POST">
              @method('put')
                @csrf
                  ...
                  <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 sm:col-span-3">
                      <label for="name" class="block text-sm font-medium text-gray-700">Categories Name</label>
                        ...
                        @enderror
                      <input required type="text" name="name" id="name" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full h-10 px-2 shadow-md sm:text-sm border-gray-300 rounded-md" value="{{ old('name', $categoryFind->name) }}">
                    </div> 
                  </div>
                </div>
